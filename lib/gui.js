...
<main>
  ...
  <div class="toolbar">
    <div class="group"><label><input type="checkbox" id="showAll"> Show all</label><label title="Fast scan checks the 7 common right-click surfaces. Full scan walks every registered file type and system handler (slower)."><input type="checkbox" id="fullScan"> Full scan</label><button id="refresh">Refresh</button><label>Theme <select id="theme" class="theme-select" aria-label="Theme"><option value="system">System</option><option value="light">Light</option><option value="dark">Dark</option></select></label></div>
...
let loadTimer=null;
async function load(){const t0=Date.now();if(loadTimer)clearInterval(loadTimer);const tick=()=>{const s=Math.round((Date.now()-t0)/1000);setStatus('Scanning the registry and verifying signatures… '+s+'s. The first scan can take up to a minute; later scans are cached and fast.');};tick();loadTimer=setInterval(tick,1000);try{[admin,classic,handlers,conflicts,meta]=await Promise.all([api('/api/admin').then(x=>x.admin),api('/api/classic-menu').then(x=>x.enabled),api('/api/handlers'),api('/api/conflicts'),api('/api/meta')]);}finally{clearInterval(loadTimer);loadTimer=null;}el('adminBanner').classList.toggle('show',!admin);el('version').textContent='v'+meta.version+' / '+meta.scope;el('classicToggle').textContent=classic?'Use Windows 11 menu':'Use classic menu';el('fullScan').checked=meta.scope==='all';populateFilters();render();setStatus('Ready','ok');}
...
el('fullScan').addEventListener('change',async event=>{try{await api('/api/scope',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({scope:event.target.checked?'all':'broad'})});await load();}catch(error){el('fullScan').checked=!event.target.checked;setStatus(error.message,'error');}});
