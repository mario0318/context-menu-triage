...
<style>
:root { color-scheme: light; --bg:#eef1f3; --panel:#fff; --line:#c9d0d5; --text:#172126; --muted:#5d6a70; --danger:#b3261e; --warn:#8a5a00; --ok:#08766b; --accent:#176b87; --ink:#26343a; }
...
button { border:1px solid #aeb8be; background:#fff; color:var(--text); border-radius:4px; padding:6px 10px; cursor:pointer; min-height:32px; white-space:nowrap; }
button:hover { border-color:var(--ink); background:#f7f9fa; }
button:focus-visible,input:focus-visible,select:focus-visible { outline:2px solid var(--accent); outline-offset:1px; }
button.primary { color:#fff; background:var(--accent); border-color:var(--accent); }
button.danger { color:#fff; background:var(--danger); border-color:var(--danger); }
button:disabled { opacity:.55; cursor:not-allowed; }
input[type="text"],input[type="search"],select { width:100%; min-width:0; border:1px solid var(--line); border-radius:4px; padding:6px 8px; min-height:34px; background:#fff; }
label { display:inline-flex; align-items:center; gap:7px; color:var(--muted); }
.banner { display:none; margin-bottom:8px; padding:8px 10px; border:1px solid #d7aa42; border-left:4px solid #d7aa42; background:#fff8e4; color:#624300; }
...
th,td { border-bottom:1px solid #dce1e4; padding:8px 9px; text-align:left; vertical-align:top; font-size:12.5px; }
th { position:sticky; top:0; background:#dde3e6; z-index:1; font-size:11px; color:#334248; text-transform:uppercase; letter-spacing:0; }
...
.pill { display:inline-block; min-width:62px; text-align:center; padding:2px 6px; border:1px solid var(--line); background:#f8fafc; }
.pill.on { color:var(--danger); border-color:#f2a29b; background:#fff1f0; }.pill.good { color:var(--ok); border-color:#9fd6b8; background:#effaf3; }.pill.warn { color:var(--warn); border-color:#f4d18b; background:#fff8e6; }
...
.registration { margin-top:5px; padding-top:5px; border-top:1px solid #e4e8ea; word-break:break-all; }
dialog { width:min(700px,calc(100vw - 28px)); border:1px solid var(--ink); border-top:4px solid var(--ink); border-radius:4px; padding:0; color:var(--text); }
dialog::backdrop { background:rgba(20,29,33,.45); }
.dialog-body { padding:16px; }.dialog-body h2 { margin:0 0 10px; font:650 17px Bahnschrift,sans-serif; }.dialog-body pre { max-height:330px; overflow:auto; white-space:pre-wrap; padding:10px; background:#f1f3f4; border:1px solid var(--line); font:11.5px/1.45 Consolas,monospace; }
.dialog-actions { display:flex; justify-content:flex-end; gap:8px; padding:10px 16px; border-top:1px solid var(--line); background:#f5f7f8; }
...
<main>
  ...
  <div class="toolbar">
    <div class="group"><label><input type="checkbox" id="showAll"> Show all</label><button id="refresh">Refresh</button></div>
...
['showAll','publisher','signature','hive','view','state'].forEach(id=>el(id).addEventListener('change',render));el('search').addEventListener('input',render);el('refresh').addEventListener('click',load);
