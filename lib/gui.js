...
.brand h1 {
  ...
  font-size:17px; font-weight:600; letter-spacing:.2px; line-height:1.05; white-space:nowrap;
}
.ver { display:block; font-size:11px; color:var(--faint); letter-spacing:.3px; }
.stats { display:flex; gap:7px; flex-wrap:wrap; flex:1 1 auto; min-width:0; }
...
select {
  height:36px; padding:0 30px 0 11px; border:1px solid var(--line); border-radius:9px; background:var(--panel-2);
  ...
  background-repeat:no-repeat; background-position:right 9px center; max-width:170px; color:var(--muted);
}
select:hover { color:var(--text); border-color:var(--ink); }
.segmented { display:inline-flex; border:1px solid var(--line); border-radius:9px; overflow:hidden; background:var(--panel-2); }
...
.txtbtn { height:36px; padding:0 12px; border:1px solid var(--line); border-radius:9px; background:var(--panel-2); color:var(--muted); display:inline-flex; align-items:center; gap:7px; font-size:12px; white-space:nowrap; }
...
<div class="app">
  <header class="topbar">
    <div class="brand">
      ...
      <div><h1>Context Menu Triage</h1><span class="ver" id="version"></span></div>
    </div>
  ...
  <div class="controls">
    ...
    <button class="iconbtn" id="exportBtn" type="button" title="Export snapshot" aria-label="Export snapshot">
    ...
    <button class="iconbtn" id="importBtn" type="button" title="Import snapshot" aria-label="Import snapshot">
      <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M12 15V3"/><path d="M7 8l5-5 5 5"/><path d="M4 21h16"/></svg>
    </button>
...
function renderAdmin(){const c=el('adminChip');if(admin){c.className='chip admin-yes';c.innerHTML='<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M12 3l7 3v5c0 4.5-3 8-7 10-4-2-7-5.5-7-10V6z"/><path d="M9 12l2 2 4-4"/></svg>Administrator';c.title='Running with administrator rights';}else{c.className='chip admin-no';c.innerHTML='<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><rect x="4" y="10" width="16" height="10" rx="2"/><path d="M8 10V7a4 4 0 0 1 8 0"/></svg>Standard \\u2014 elevate';c.title='Click to relaunch as administrator';}}
...
async function toggle(clsid,act){
  ...
  try{
    if(act==='block')await jpost('/api/block',{clsid:clsid,name:r.name||r.label});
    else await jpost('/api/unblock',{clsid:clsid});
...
el('exportBtn').addEventListener('click',async()=>{try{const o=await jpost('/api/export',{file:el('filePath').value.trim()||undefined});el('filePath').value=o.file;setStatus('Exported '+o.count+' handlers to '+o.file,'ok');}catch(e){setStatus(e.message,'error');}});
